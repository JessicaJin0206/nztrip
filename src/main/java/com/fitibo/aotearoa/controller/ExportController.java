...
public class ExportController extends AuthenticationRequiredController {
  ...
  public static void main(String[] args) {
    ...
    Observable.range(0, 10).groupBy(input -> input % 2).subscribe(groupedObservable -> {
      groupedObservable.reduce((v1, v2) -> {
        System.out.println("v1:" + v1 + " v2:" + v2);
        return v1 + v2;
      }).collect(() -> result, (map, value) -> map.put(groupedObservable.getKey(), value)).subscribe();
    });
